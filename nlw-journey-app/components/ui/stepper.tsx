    ...
    return React.useMemo(() => {
        ...
        return (
            <span
                ...
                key="label"
                className={cn(
                    'font-medium text-center text-md',
                    isCurrentStep ? 'dark:text-white text-zinc-950' : 'text-zinc-950 dark:text-white'
                )}
            >
                {(index || 0) + 1}
            </span>
...
const StepLabel = ({ isCurrentStep, opacity, label, description }: StepLabelProps) => {
    ...
    return shouldRender ? (
        <div
            aria-current={isCurrentStep ? 'step' : undefined}
            className={cn(
                'stepper__step-label-container dark:text-white text-zinc-950',
                'flex-col flex',
